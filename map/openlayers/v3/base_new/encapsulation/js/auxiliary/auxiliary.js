...
auxiliary.prototype = {
		nullBool : function(param){  // 对象为空判断 为空返回true，反之false
		...
		} , isInteger : function(theObj) {//验证字符串是否是整数
			  return typeof theObj === 'number' && theObj%1 === 0;
		...
		} , arrForEach : function(arr , func) {	// 经测试，双重循环100次（10000次循环）比for循环 差 0.5秒
			try{
	  ...
	  } , checkNumber : function(theObj) {//验证字符串是否是数字
		  return !isNaN(theObj*1);
	  } , checkInteger : function(theObj) {//验证字符串是否是数字
		  return !isNaN(theObj*1);
		  ...
		  return false;
	  } , getSelectAttr : function(select , attrName){	// 获取select子类的指定属性值
		  ...
		  return s;
	  } , getContent : function(str){	// 获取 括号 中的内容
		  var p=/\(([^()]+)\)/g;
		  return p.exec(s);
	  }
}
