...
!function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
         ...
         * @returns
         */
        var createAddFunctionPopup = function( bodyName ) {
    		var $_div1 = $("<div>").addClass("modal fade in quickqueryInterface").attr({ id:"addButtonPopup",tabindex:"2",role:"dialog","aria-labelledby":"myModalLabel","aria-hidden":"true" })
    		var $_div2 = $("<div>").addClass("modal-dialog");
    		var $_div3 = $("<div>").addClass("modal-content");
    		var $_div4 = $("<div>").addClass("modal-header");
    		var $_button1 = $("<button>").addClass("close").attr({ type:"button","data-dismiss":"modal","aria-hidden":"true" }).text("&times;");
    		var $_hfore1 = $("<h4>").addClass("modal-title").text("快捷查询管理");
    		var $_div5 = $("<div>").addClass("modal-body");
    		var $_input1 = $("<input>").attr({type:"text"}).addClass("invalid").attr({"id":bodyName,"placeholder":"请输入15个字符以内.."});
    		var $_div7 = $("<div>").addClass("modal-footer");
    		var $_button2 = $("<button>").addClass("btn btn-default").attr({type:"button","data-dismiss":"modal"}).text("关闭");
    		var $_button3 = $("<button>").addClass("btn btn-primary").attr({type:"button"}).text("提交");
    		$("body").append(
    				$_div1.append(
    				$_div2.append(
    				$_div3.append(
    					$_div4.append($_button1).append($_hfore1)
    				).append(
    					$_div5.append($_input1)
    				).append(
    					$_div7.append($_button2).append($_button3)
    				)
    			))
    		);
    	}
        ...
        if (this.options.addButton) {
        	this.$toolbar.append("<button class='btn btn-red addButton' data-toggle='modal' data-target='#addButtonPopup'>增加</button>");
        	createAddFunctionPopup("addInputText");
        }
