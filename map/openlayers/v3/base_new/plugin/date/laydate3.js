		...
		var n = this,
			...
			o = {
				...
				aWeek: function(event) {
					var a = n.config , str = "" , date = new Date();
					if( null !== a.type ){
						var end = date.Format(a.format);
						var start = new Date(date.setDate(date.getDate()-7)).Format(a.format);
						str = start + " " +a.range + " " + end;
					}else{
						str = new Date(date.setDate(date.getDate()-7)).Format(a.format);
					}
					n.setValue(str).remove()
				},
				oneMonth: function() {
					var a = n.config , str = "" , date = new Date();
					if( null !== a.type ){
						var end = date.Format(a.format);
						var start = new Date(date.setMonth(date.getMonth()-1)).Format(a.format);
						str = start + " " +a.range + " " + end;
					}else{
						str = new Date(date.setMonth(date.getMonth()-1)).Format(a.format);
					}
					n.setValue(str).remove()
				},
				halfYear: function() {
					...
					if( null !== a.type ){
						var end = date.Format(a.format);
						var start = new Date(date.setMonth(date.getMonth()-6)).Format(a.format);
						str = start + " " +a.range + " " + end;
					}else{
						str = new Date(date.setMonth(date.getMonth()-6)).Format(a.format);
					}
					n.setValue(str).remove()
				}
			};
